...
/*        try {
            DataInput dataInput = new DataInputStream(new FileInputStream("C:\\Users\\Jonatan\\workspace\\EvolutionRunner\\src\\in.txt"));

            System.out.println(state.population);
            System.out.println(((GPIndividual)state.population.subpops[0].individuals[0]).trees[0]);
            GPIndividual gimp = ((GPIndividual) state.population.subpops[0].individuals[0]).lightClone();
            gimp.readGenotype(state, dataInput);
            state.population.subpops[0].individuals[0] = gimp;
            System.out.println(((GPIndividual) state.population.subpops[0].individuals[0]).toString());
